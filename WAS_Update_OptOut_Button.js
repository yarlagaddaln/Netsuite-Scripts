function Update_Client_Opt_Out() {
	...
	if (search.length >= 1) {
		...
		for (var i in search) {
			...
			if (salesorder_no) {
				...
				nlapiSubmitField('salesorder',salesorder_no,'custbody_client_opted_out','T');
			}
			...
			if (linked_opp) {
				...
				nlapiSubmitField('opportunity',linked_opp,'custbody41',4);
			} else {
			...
			if (i == 0) {
				...
				nlapiSubmitField('customrecord_model_acv',macv_id,'custrecord_macv_opt_out','T');

			}
			
			//update the acv records fields
				nlapiLogExecution('debug', 'update acv', search[i].getId());
			nlapiSubmitField('customrecord184',search[i].getId(),'custrecord_acv_opt_out','T');
